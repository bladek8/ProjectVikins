...
namespace Assets.Script.DAL
{
    public class MVC_Game2Context
    {
        public static void GetSave()
        {
            ...
            foreach (var file in files)
            {
                ...
                if (file.Name == "Player.dat")
                {
                    BinaryFormatter bf = new BinaryFormatter();
                    FileStream __file = File.Create(file.FullName);
                    player = new List<Player>() { new Player() { PlayerId = 1, AttackMax = 5, AttackMin = 2, CharacterTypeId = 1, InitialX = -27, InitialY = 0, SpeedWalk = 1.5f, SpeedRun = 2, IsBeingControllable = true, LastMoviment = Helpers.PossibleMoviment.None, Life = 5, PlayerMode = Helpers.PlayerModes.Follow, X = -27, Y = 0 }, new Player() { PlayerId = 2, AttackMax = 5, AttackMin = 2, CharacterTypeId = 1, InitialX = 1.15f, InitialY = -25, SpeedWalk = 3, SpeedRun = 2, IsBeingControllable = false, LastMoviment = Helpers.PossibleMoviment.None, Life = 5, PlayerMode = Helpers.PlayerModes.Follow, X = -25, Y = 3 }, new Player() { PlayerId = 3, AttackMax = 2, AttackMin = 1, CharacterTypeId = 1, InitialX = -25, InitialY = 0, SpeedWalk = 3, SpeedRun = 2, IsBeingControllable = false, LastMoviment = Helpers.PossibleMoviment.None, Life = 5, PlayerMode = Helpers.PlayerModes.Follow, X = -25, Y = 0  } };
        ...
        public static readonly Player defaultPlayer = new Player() { PlayerId = players.Count, PlayerMode = Helpers.PlayerModes.Follow, IsBeingControllable = false, AttackMin = 2, AttackMax = 4, LastMoviment = Helpers.PossibleMoviment.None, Life = 3, SpeedRun = 2, SpeedWalk = 2, CharacterTypeId = 1 };
        public static readonly Enemy defaultEnemy = new Enemy() { EnemyId = enemies.Count, AttackMin = 2, AttackMax = 4, LastMoviment = Helpers.PossibleMoviment.None, Life = 1, SpeedRun = 2, SpeedWalk = 2, CharacterTypeId = 1 };
    }
}
